...
public class HttpServer
{
    ...
    private bool MethodHandler(HttpListenerContext _httpContext)
    {
        ...
        var controller = assembly.GetTypes().Where(t => Attribute.IsDefined(t, typeof(HttpController))).FirstOrDefault(c => c.GetCustomAttribute<HttpController>().Route == null ? c.Name.ToLower().Equals(controllerName.ToLower()) : c.GetCustomAttribute<HttpController>().Route.ToLower().Equals(controllerName.ToLower()));

        if (controller == null) return false;

        var method = controller.GetMethods()
                .Where(m => m.GetCustomAttributes().Any(attr => attr.GetType().Name == $"Http{_httpContext.Request.HttpMethod}")).FirstOrDefault(t => strParams.Length > 0 ? t.GetParameters().Length == strParams.Length : t.GetCustomAttribute<HttpMethodAttribute>().Route == null);
